...
def make_audio_features(path, save_to_local_file=False):
...
def make_audio_dataset(path_list = [], save_to_local_file=False):
    temp = []
    i=0
    for path in path_list:
        features = make_audio_features(path, save_to_local_file=save_to_local_file)
...
train_ds = CustomDataGen(df.head(2), batch_size=1)
