...
for i in [1, 2, 3]:
    fold = i
    print("Fold", i, "-"*20)
    model = tf.keras.models.load_model(f"/raid/home/labusermoctar/ptsd_images_detection/saved_models/ptsd_model_video_{i}.h5")
    ...
    print(pred_values)
    df = pd.read_csv(FOLDS_TEST[i-1])

    true_y = np.array(df["class"])
    print("Classification report of fold", i)
    print(classification_report(true_y, pred_values, target_names=classes, digits=4))
